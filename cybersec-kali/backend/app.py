...
from license import license_manager, require_license, require_feature, generate_license_key

# Import advanced tools detection
try:
    from advanced_tools_api import create_advanced_tools_endpoint
    ADVANCED_DETECTION_AVAILABLE = False  # Geçici olarak kapalı
except ImportError:
...
# Flask-CORS tüm endpointlerde otomatik CORS header'ı ekler
...
# Add advanced tools detection endpoint if available
...
def _get_crypto_key() -> bytes:
    raw = os.environ.get('TERMINAL_SECRET_KEY') or os.environ.get('SECRET_KEY') or app.config.get('SECRET_KEY')
    if not raw:
...
def audit_logs_export():
    ...
    response = app.response_class(json.dumps(entries), mimetype='application/json')
...
@app.route('/api/monitoring/targets/summary', methods=['GET'])
@admin_ip_required
@rate_limit_admin(limit=30, window_sec=60)
