...
pub async fn create_report(
    ...
    // Fetch org branding for white-label reports
    let org_branding: Option<(String, Option<String>, Option<bool>, Option<String>)> = sqlx::query_as(
        "SELECT primary_color, secondary_color, hide_platform_logo, custom_footer_text FROM organizations WHERE id = $1"
    )
    .bind(org_id)
    .fetch_optional(&state.db)
    .await
    ...
    let html_content = generate_html_report(
        ...
        primary_color.as_deref(), secondary_color.as_deref(), hide_platform_logo.unwrap_or(false), custom_footer_text.as_deref(),
...
fn generate_html_report(
    ...
    primary_color: Option<&str>, secondary_color: Option<&str>, hide_platform_logo: bool, custom_footer_text: Option<&str>,
    ...
    let logo_html = if let Some(data_uri) = org_logo_data_uri {
        ...
        if hide_platform_logo {
            format!(r#"<div class="header-logo"><img class="org-logo" src="{}" alt="{}" /></div>"#, data_uri, org_display)
        } else {
            format!(
    ...
    let brand_css = if let Some(p) = primary_color {
        format!(":root {{ --color-primary: {}; }}", p)
    } else {
        String::new()
    };
    let accent_css = if let Some(s) = secondary_color {
        format!(":root {{ --color-accent: {}; }}", s)
    } else {
        String::new()
    };
    let footer_html = custom_footer_text.as_deref().unwrap_or_default();
    let custom_footer_section = if footer_html.is_empty() {
        String::new()
    } else {
...
<div class="cover">
    ...
    <div class="cover-footer">
        <p>Generated by {generator} | cyber-sec-pro.com</p>
        <p>© 2026 {generator}. All rights reserved.</p>
...
<div class="content">
    ...
    <div class="footer">
        <p>Generated by <strong>{generator}</strong> | <a href="https://cyber-sec-pro.com" target="_blank">cyber-sec-pro.com</a></p>
        ...
        generator = generator,
        custom_footer_section = custom_footer_section,
    )
}
