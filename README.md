...
- **Product tool count: 89 (14 categories).** The number shown and runnable in the app is the set of **active, curated tools** (`SELECT COUNT(*) FROM tools WHERE is_active = TRUE` → 89), and every one of them has a working parameter form. The wider **catalogue** seeds ~1,510 rows for search/coverage, but most are inactive (missing binary / broken / needs-interactive) and are **not** exposed in the UI — so "89" is the honest figure to quote, not the raw catalogue size. `frontend/src/lib/catalog-counts.ts` is the single source of truth.
