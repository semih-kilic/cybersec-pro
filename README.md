...
- **Honesty audit across every user-facing surface.** A sweep to make the product describe itself truthfully removed claims the code does not support. Both Trust Centers (`frontend/src/components/pages/SecurityTrustCenter.tsx`, `saas-frontend/src/pages/TrustCenterPage.tsx`) had asserted HSM key storage, 90-day key rotation, WebAuthn, ABAC, WORM/SIEM, automated SBOM, ML anomaly detection/IDS-IPS, dedicated Enterprise instances, and third-party penetration tests — none of which exist. Corrected to what the code does, with the verified allow/deny list recorded in **CLAUDE.md §16**. The generated security report no longer claims certifications we do not hold, is no longer "signed" by a fictitious analyst, and carries a real SHA-256 content digest. The `/security` marketing page and the homepage hero badge ("Trusted by Security Teams", with zero paying customers) were corrected the same way.
- **AI discoverability.** The origin-level and Cloudflare AI-crawler blocks (GPTBot, ClaudeBot, CCBot, Google-Extended) were **removed** so the product can appear accurately in AI answers — reversing the August default noted below. Shipped `llms.txt`/`llms-full.txt`, a generated `sitemap.xml` (1,130 URLs, hreflang alternates, zero dead links, replacing a hand-maintained file that was 969/1918 404s), IndexNow submission, and per-tool + pricing structured data. React hydration error #418 (Cloudflare Email Obfuscation rewriting bare-text emails) fixed via `frontend/src/components/Email.tsx`.
- **Marketing i18n at parity.** `frontend/src/i18n/messages/{en,tr,de,fr,es,ar,ja,zh,ru,ko}.json` — 414 keys, all ten locales complete. Eight were missing ~half their keys (the whole Trust Center, pricing page, comparison table); because `src/i18n/request.ts` deep-merges to English, the gap rendered silently as English and had to be measured, not observed. A real pricing page (previously a 404) was built with the limits `rust-backend/src/services/plan.rs` actually enforces.
- **Blog rewrite.** All ten posts rewritten from 150–430-word command dumps to 1,200–1,900-word guides (nmap, sqlmap, hashcat ×2, Metasploit ×2 split into first-exploit + post-exploitation, wireless, Wireshark, OWASP, CI/CD), each verified against the tools installed on the host. Fixed `renderMarkdown` so markdown tables render at all, corrected read times to computed values, and fixed a CI/CD sample that called a wrong API path.
- **Tool-parameter handling unified.** `saas-frontend/src/lib/toolFormParams.ts` is now the single flattener for tool form parameters (21 unit tests). The scan execution page had drifted from the tool detail page and, as a result, rendered credential fields in plain text and could never show the "Accepted Target Types" panel; both fixed.
- **Feed link hardening.** `services/news_feed.rs` now rejects non-http(s) links from third-party RSS feeds before they reach an `href` (3 tests), and the dead, never-shipped `TestimonialsSection` — which fetched feeds through an unvetted public CORS proxy — was deleted.

## What's New (August 2026)

- **Product tool count: 88 (14 categories).** The number shown and runnable in the app is the set of **active, curated tools** (`SELECT COUNT(*) FROM tools WHERE is_active = TRUE` → 88), and every one of them has a working parameter form. The wider **catalogue** seeds ~1,510 rows for search/coverage, but most are inactive (missing binary / broken / needs-interactive) and are **not** exposed in the UI — so "88" is the honest figure to quote, not the raw catalogue size. `frontend/src/lib/catalog-counts.ts` is the single source of truth.
...
- **Domain unification (August 2026).** All references, OAuth callbacks, dashboard URLs, mail links and SEO metadata standardised on **`cyber-sec-pro.com`** (app., www., api. subdomains). Sitemap expanded to include marketing pages. (The AI-crawler blocks this line once described were removed in September 2026 — see below.)
