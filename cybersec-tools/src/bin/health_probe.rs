...
fn truncate(s: &str) -> String {
    ...
    if s.len() <= EVIDENCE_MAX {
        s
    } else {
        format!("{}...[trunc]", &s[..EVIDENCE_MAX])
    }
}

async fn probe_binary(
    binary_name: &str,
    custom_probe: Option<&str>,
    timeout: Duration,
                ...
                Ok(mut child) => {
                    match tokio::time::timeout(timeout, child.wait_with_output()).await {
                        Ok(Ok(output)) => {
                            ...
                            let status = if output.status.code() == Some(0) { "ok" } else { "broken" };
                            return (status.into(), output.status.code(), ev);
                        }
                        Ok(Err(e)) => return ("broken".into(), None, truncate(&e.to_string())),
                        Err(_) => {
                            let _ = child.kill().await;
                            return ("timeout".into(), None, format!("timeout {}s", timeout.as_secs()));
                        }
                    }
                }
...
async fn main() -> Result<()> {
    ...
    let rows: Vec<crate::tools::Tool> = sqlx::query_as(
        "SELECT id, name, category, binary_name, command_template, is_active,
         ...
         WHERE binary_name IS NOT NULL AND binary_name <> ''
           AND (last_health_check IS NULL OR last_health_check < NOW() - INTERVAL '24 hours')
         ORDER BY name"
    )
    .fetch_all(&pool)
    .await?;
    ...
    for row in rows {
        ...
        let bin = row.binary_name.clone().unwrap_or_default();
        let probe = row.health_probe.clone();
        let id = row.id.clone();
        ...
        handles.push(tokio::spawn(async move {
            ...
            pb.inc(1);
            if pb.position() % 50 == 0 || pb.position() == total as u64 {
    ...
    for handle in handles {
        let (id, name, bin, status, exit_code, evidence) = handle.await?;
        *summary.entry(status.clone()).or_insert(0) += 1;
        detailed.push(serde_json::json!({
            "id": id, "name": name, "binary": bin,
            "status": status, "exit_code": exit_code, "evidence": evidence
        }));
    ...
    println!("[probe] DONE in {:.1}s", duration);
    println!("[probe] summary: {}", serde_json::to_string(&summary)?);
